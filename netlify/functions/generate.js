// ============================================================
// FounderTxt — Netlify Serverless Function
// Handles POST /api/generate
// ============================================================

// ── Patterns (same as Vercel api/generate.js) ──────────────
...
exports.handler = async function (event) {
  ...
  if (!patternId || !answers) {
    return {
      statusCode: 400,
      headers: CORS_HEADERS,
      body: JSON.stringify({ error: 'Missing patternId or answers' }),
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers: CORS_HEADERS,
      body: JSON.stringify({ result, pattern: patternId }),
    };
  } catch (err) {
    ...
    return {
      statusCode: 502,
      headers: CORS_HEADERS,
      body: JSON.stringify({ error: 'AI generation failed. Please try again.' }),
    };
  }
};
